...
def report(result):
    ...
    comb_str = f"rank {rank_c+1}/{n}" if rank_c is not None else "MISSING"
    photo_str = f"rank {rank_p+1}/{n_photo}" if rank_p is not None else "MISSING/FILTERED"

    photo_fixed = (rank is not None and rank_p is not None and rank_p < rank)
    photo_tag = " ✓ FIXED" if rank_p == 0 else (f" partial (rank {rank_p+1})" if photo_fixed else " ✗")

    print(f"\n{'='*70}")
    print(f"  {cam}  |  {n} total → {n_photo} pass photo filter")
    print(f"  acc-rank: {acc_str}   photo-rank: {photo_str}{photo_tag}")
    print(f"  {gt_str}")
    print(f"{'='*70}")
    ...
    if rank is not None:
        ...
        print(f"  Sphere:  acc={sphere['acc']:.3f}  cont={sphere['continuity']:.2f}  "
              f"R/G={rg_str}  B/G={bg_str}  r/w={sphere['r_w']:.3f}  "
              f"photo={'PASS' if sphere['passes_photo'] else 'FAIL'}")

    if rank_p is not None and rank_p > 0:
        print(f"\n  Still above sphere after photo filter (top 5):")
        for i, c in enumerate(result["candidates_photo"][:rank_p][:5]):
            ...
            print(f"    [{i+1:2d}] cx={c['cx']/1:.0f} cy={c['cy']/1:.0f} "
                  ...
                  f"cont={c['continuity']:.2f}  R/G={rg_str}  B/G={bg_str}  r/w={c['r_w']:.3f}")
    elif rank is not None and rank > 0 and rank_p == 0:
        print(f"  → Photo filter moves sphere to rank 1.")
...
def summary(results):
    ...
    print(f"  SUMMARY")
    print(f"{'='*70}")
    print(f"  {'Camera':<14} {'Total':>6} {'Photo':>6} {'Acc-Rank':>10} {'Photo-Rank':>11} {'Fixed?':>7}")
    print(f"  {'-'*14} {'-'*6} {'-'*6} {'-'*10} {'-'*11} {'-'*7}")
    for r in results:
        cam = (r["camera"] or "?")[:14]
        n = r["n_candidates"]
        np_ = r["n_photo"]
        rank = r["sphere_rank"]
        rank_p = r["sphere_rank_photo"]
        acc_str   = f"{rank+1}/{n}"    if rank   is not None else "MISSING"
        photo_str = f"{rank_p+1}/{np_}" if rank_p is not None else "MISS/FILT"
        fixed = "✓" if rank_p == 0 else ("—" if rank == 0 else "✗")
        print(f"  {cam:<14} {n:>6} {np_:>6} {acc_str:>10} {photo_str:>11} {fixed:>7}")

    # Stats on how many candidates survive photo filter
    total_before = sum(r["n_candidates"] for r in results)
    total_after  = sum(r["n_photo"]      for r in results)
    print(f"\n  Photo filter: {total_before} → {total_after} candidates "
    ...
    failures_photo = sum(1 for r in results if r["sphere_rank_photo"]  is None     or r["sphere_rank_photo"]  > 0)
    print(f"  Acc-sort failures:   {failures_acc}/{len(results)}")
    print(f"  Photo-sort failures: {failures_photo}/{len(results)}")
