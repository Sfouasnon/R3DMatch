...
from r3dmatch.calibration import calibrate_card_path, calibrate_color_path, calibrate_exposure_path, calibrate_sphere_path, center_crop_roi, detect_gray_card_roi, load_calibration, load_card_roi_file, load_color_calibration, load_exposure_calibration, measure_card_from_roi, measure_color_region, measure_sphere_from_roi, solve_neutral_gains
...
def test_analyze_path_writes_manifest_and_sidecar(tmp_path: Path) -> None:
    ...
    assert clip_summary["group_key"] in {"G007_D060", "G007_D061"}
...
def test_analysis_uses_group_key_not_legacy_camera_group(tmp_path: Path) -> None:
    ...
    assert by_clip["G007_D060_0324M6_001"]["group_key"] == "G007_D060"
    assert by_clip["G007_D061_0324M6_002"]["group_key"] == "G007_D061"
...
def test_analyze_uses_exposure_calibration_only(tmp_path: Path) -> None:
    ...
    assert summary["exposure_calibration"]["group_baselines"]["G007_D060"] == pytest.approx(0.4)
...
def test_analyze_uses_color_calibration_only(tmp_path: Path) -> None:
    ...
    assert summary["color_calibration"]["group_gains"]["G007_D060"]["g"] == pytest.approx(1.0)
...
def test_analyze_uses_both_exposure_and_color_calibration(tmp_path: Path) -> None:
    ...
    assert summary["exposure_calibration"]["group_baselines"]["G007_D060"] == pytest.approx(0.4)
    assert summary["color_calibration"]["group_gains"]["G007_D060"]["r"] == pytest.approx(1.3333333)
