...
def analyze_tiff(tiff_path, profile):
    ...
    # Also rank by combined score: acc * continuity
...
def report(result):
    ...
    rank_c = result["sphere_rank_combined"]
    gt = result["gt_det"]
    w = result["w"]
    ...
    acc_rank_str  = f"rank {rank+1}/{n}"  if rank  is not None else "MISSING"
    comb_rank_str = f"rank {rank_c+1}/{n}" if rank_c is not None else "MISSING"

    fixed = (rank is not None and rank_c is not None and rank_c < rank)
    tag = " ✓ FIXED BY COMBINED SCORE" if fixed else (" ✗ STILL BROKEN" if rank_c != 0 and rank_c is not None else "")

    print(f"\n{'='*70}")
    print(f"  {cam}  |  {n} candidates")
    print(f"  acc-rank: {acc_rank_str}   combined-rank: {comb_rank_str}{tag}")
    print(f"  {gt_str}")
    print(f"{'='*70}")

    if rank is not None:
        sphere = result["candidates"][rank]
        combined_score = sphere["acc"] * sphere["continuity"]
        print(f"  Sphere:  acc={sphere['acc']:.3f}  cont={sphere['continuity']:.2f}  "
              f"combined={combined_score:.3f}  r/w={sphere['r_w']:.3f}")

    # Only show what beats sphere if it's not already rank 1 in combined
    if rank_c is not None and rank_c > 0:
        print(f"\n  Still above sphere in combined sort:")
        for i, c in enumerate(result["candidates_combined"][:rank_c]):
            cs = c["acc"] * c["continuity"]
            print(f"    [{i+1:2d}] cx={c['cx']/1:.0f} cy={c['cy']/1:.0f} "
                  f"r={c['r']/1:.0f}  acc={c['acc']:.3f}  "
                  f"cont={c['continuity']:.2f}  combined={cs:.3f}  r/w={c['r_w']:.3f}")
    elif rank is not None and rank > 0 and rank_c == 0:
        print(f"  → Combined score moves sphere to rank 1.")
...
def summary(results):
    ...
    print(f"  SUMMARY")
    print(f"{'='*70}")
    print(f"  {'Camera':<14} {'Cands':>6} {'Acc-Rank':>10} {'Comb-Rank':>10} {'Fixed?':>7}")
    print(f"  {'-'*14} {'-'*6} {'-'*10} {'-'*10} {'-'*7}")
    for r in results:
        cam = (r["camera"] or "?")[:14]
        n = r["n_candidates"]
        rank = r["sphere_rank"]
        rank_c = r["sphere_rank_combined"]
        acc_str  = f"{rank+1}/{n}"  if rank  is not None else "MISSING"
        comb_str = f"{rank_c+1}/{n}" if rank_c is not None else "MISSING"
        fixed = "✓" if (rank is not None and rank_c is not None and rank_c < rank) else ("—" if rank == 0 else "✗")
        print(f"  {cam:<14} {n:>6} {acc_str:>10} {comb_str:>10} {fixed:>7}")
    ...
    if sphere_cands and non_sphere:
        sp_scores  = [c["acc"] * c["continuity"] for c in sphere_cands]
        non_scores = [c["acc"] * c["continuity"] for c in non_sphere]
        print(f"\n  Combined score (acc×cont) — spheres:     "
              f"mean={np.mean(sp_scores):.3f}  min={np.min(sp_scores):.3f}  max={np.max(sp_scores):.3f}")
        print(f"  Combined score (acc×cont) — non-spheres: "
              f"mean={np.mean(non_scores):.3f}  min={np.min(non_scores):.3f}  max={np.max(non_scores):.3f}")

    failures = [r for r in results if r["sphere_rank_combined"] is not None and r["sphere_rank_combined"] > 0]
    print(f"\n  Failures remaining with combined sort: {len(failures)}/{len(results)}")
